...
export async function routeQuery(userQuery, conversationHistory = []) {
  ...
  const prompt = `
You are a lightweight Router. For the given user query and recent conversation context:
...
Return ONLY valid JSON: { "refined_query":"...", "intent":"...", "format":"..." }.
...
`;
  ...
  try {
    ...
    console.warn("routeQuery error:", e?.message || e);
    return { refined_query: userQuery, intent: "question", format: "short_answer" };
  }
}
