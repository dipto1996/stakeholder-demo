...
End by listing a JSON block mapping [n] -> {id, title, url} so the UI can associate citations.
${formatInstruction}
`;
...
`;
  ...
  try {
    ...
    // Attempt to extract trailing JSON mapping if present
    // We'll return the raw markdown as answer and pass source metadata separately.
